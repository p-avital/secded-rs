...
//  Since `SecDedDynamic` can't be easily represented in C, and has a sepecialized `free` operation,
...
//  Droping SECDED_DYN is non-trivial and should be done using `SECDED_DYN_free()`
SECDED_DYN *SECDED_DYN_new(size_t encodable_bits);

//  Deallocates `secded`'s internals and secded itself.
void SECDED_DYN_free(SECDED_DYN *secded);
